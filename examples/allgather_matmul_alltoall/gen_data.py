...
def gen_and_save_data(M, N, K, rank_size, dtype=np.float16):
    ...
    if not debug:
        ...
        a_ele_num = M * K
        b_ele_num = K * N_per_rank
        all_A = [
            # np.arange(rank * a_ele_num, (rank + 1) * a_ele_num).reshape(M, K)
            np.ones(shape=[M, K], dtype=np.float16)
            for rank in range(rank_size)
        ]
        b_start_val = M * K * rank_size
        all_B = [
            # np.arange(
            #     b_start_val + rank * b_ele_num, b_start_val + (rank + 1) * b_ele_num
            # ).reshape(K, N_per_rank)
            np.ones(shape=[K, N_per_rank], dtype=np.float16)
            for rank in range(rank_size)
        ]
    ...
    print(f'after AG, {A_gathered.shape=}, A_gathere=')
    pp(A_gathered)
    ...
    for i in range(rank_size):
        ...
        print(f'after mm, {C_partial_i.shape=} C[{i}]=')
        pp(C_partial_i)
    ...
    for j in range(rank_size):
        ...
        if C_golden_j.shape != (M, N):
            raise ValueError(
                f"Shape mismatch for golden output on rank {j}. Expected {(M,N)}, got {C_golden_j.shape}"
            )

        C_golden_j.tofile(os.path.join(output_dir, f"golden_rank{j}.bin"))
        print(f'after a2a: {C_golden_j.shape=}  C[{j}]=')
        pp(C_golden_j)
        print(f"Saved golden_rank{j}.bin with shape {C_golden_j.shape}, {C_golden_j.dtype=}")

    print(
        "Successfully generated all input and golden files based on Matmul-Scatter logic."
    )

    if debug:
        B_gathered = np.stack(all_B, axis=1).reshape((K, N))
        print("full B=")
        pp(B_gathered)
        print(f'Derect mul:')
        C0 = all_A[0] @ B_gathered
        C1 = all_A[1] @ B_gathered
        print("C0=")
        pp(C0)   
        print("C1=")
        pp(C1)
