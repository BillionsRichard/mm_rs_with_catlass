...
#ifndef CATLASS_EPILOGUE_BLOCK_EPILOGUE_PER_TENSOR_DEQUANT_HPP
#define CATLASS_EPILOGUE_BLOCK_EPILOGUE_PER_TENSOR_DEQUANT_HPP
...
using Catlass::Epilogue::Block::BlockEpilogue;
using Catlass::Gemm::GemmType;
using Catcoc::CommEpilogue::EpilogueAtlasA2PerTensorDequant;
...
>
class BlockEpilogue <
    EpilogueAtlasA2PerTensorDequant<UB_STAGES_>,
    CType_,
    ScaleType_,
    DType_,
...
> {
public:
    using DispatchPolicy = EpilogueAtlasA2PerTensorDequant<UB_STAGES_>;
    ...
    static_assert(
        std::is_same_v<ElementC, int32_t> && (std::is_same_v<ElementD, half> || std::is_same_v<ElementD, bfloat16_t>) &&
            std::is_same_v<ElementScale, ElementD>,
    ...
    struct Params {
        // PerTensorScaleType perTensorScale{1.0};
...
public:
    ...
    struct Params {
        ...
        CATLASS_DEVICE
        Params(
            // PerTensorScaleType perTensorScale_, 
            __gm__ ElementScale *ptrScale_, LayoutScale const &layoutScale_,
            __gm__ ElementD *ptrD_, LayoutD const &layoutD_
        ) : ptrScale(ptrScale_), layoutScale(layoutScale_),
            ptrD(ptrD_), layoutD(layoutD_) {}
...
#endif  // CATLASS_EPILOGUE_BLOCK_EPILOGUE_PER_TENSOR_DEQUANT_HPP
