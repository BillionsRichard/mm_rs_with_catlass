 ...
 */
#ifndef CATCOC_DGEMM_KERNEL_ALLGATHER_MATMUL_LOCAL_MM_HPP
#define CATCOC_DGEMM_KERNEL_ALLGATHER_MATMUL_LOCAL_MM_HPP
...
public:
    ...
    void operator()<AscendC::AIC>(Params &params)
    {
        ...
        //// Local matmul
        typename BlockScheduler::Base localMatmulBlockScheduler(params.problemShape, blockShape.GetCoordMN());
        uint32_t localCoreLoops = localMatmulBlockScheduler.GetCoreLoops();

        for (uint32_t loopIdx = aicoreIndex; loopIdx < localCoreLoops; loopIdx += aicoreNum) {
            // Compute block location
            GemmCoord blockCoord = localMatmulBlockScheduler.GetBlockCoord(loopIdx);
            GemmCoord actualBlockShape = localMatmulBlockScheduler.GetActualBlockShape(blockCoord);

            // Compute initial location in logical coordinates
            GemmCoord offsetCoord = blockCoord * blockShape;
            MatrixCoord blockOffsetA = offsetCoord.GetCoordMK();;
            MatrixCoord blockOffsetB = offsetCoord.GetCoordKN();;
            MatrixCoord rankOffsetC = params.problemShape.GetCoordMN() * Catlass::MakeCoord<uint32_t>(params.rankIdx, 0);
            MatrixCoord blockOffsetC = rankOffsetC + offsetCoord.GetCoordMN();;
        ...
        //// Remote matmul
        BlockScheduler matmulBlockScheduler(params.rankSize - 1, params.commInterval, params.problemShape, blockShape.GetCoordMN());

        auto mLoops = CeilDiv(matmulBlockScheduler.GetMLoops(), params.rankSize - 1);
        auto nLoops = matmulBlockScheduler.GetNLoops();
        auto blockPerComm = params.commInterval * (params.rankSize - 1) * nLoops;
        ...
        for (uint32_t commIdx = 0; commIdx < commLoops; ++commIdx) {
            ...
            auto mLoopsPerRank = actualBlocksPerComm / ((params.rankSize - 1) * nLoops);
            ...
            for (uint32_t compIdx = aicoreIndex; compIdx < actualBlocksPerComm; compIdx += aicoreNum) {
                ...
                // Compute block-scoped matrix multiply-add
                mmad(
                    gmAShmem[offsetA], params.layoutA,
                    gmB[offsetB], params.layoutB,
                    gmC[offsetC], params.layoutD,
                    actualBlockShape);
            }
...
#endif // CATCOC_DGEMM_KERNEL_ALLGATHER_MATMUL_LOCAL_MM_HPP
