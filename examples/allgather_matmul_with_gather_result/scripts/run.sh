...
cd ${PROJECT_ROOT}/examples/allgather_matmul_with_gather_result/

DATA_DIR=`realpath ./out`
echo "DATA_DIR: $DATA_DIR"
EXEC_BIN=${PROJECT_ROOT}/build/bin/allgather_matmul_with_gather_result
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    rm -rf ./out/*.bin
    python3 ${UTILS_PATH}/gen_data.py 4 1 ${RANK_SIZE} ${M} ${N} ${K} 0 0 ${DATA_DIR}
