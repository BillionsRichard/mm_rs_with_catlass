...
cd ${PROJECT_ROOT}/examples/matmul_reduce_scatter_quant_perchn/
EXEC_BIN=${PROJECT_ROOT}/build/bin/matmul_reduce_scatter_quant_perchn
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    # rm -rf output/*.bin
    python3 ${UTILS_PATH}/gen_data_quant_perchn.py 2 1 ${RANK_SIZE} ${M} ${N} ${K} 0 0 1 1 2

    # Set necessary parameters
    IPPORT="tcp://127.0.0.1:4853${test_idx: -1}"
