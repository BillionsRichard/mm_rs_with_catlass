...
void TestShmemInit(int rankId, int n_ranks, uint64_t local_mem_size) {
...
void TestShmemInitAttrT(int rankId, int n_ranks, uint64_t local_mem_size) {
    uint32_t deviceId = rankId % testGNpuNum + testFirstNpu;
    int status = SHMEM_SUCCESS;
    EXPECT_EQ(aclInit(nullptr), 0);
    EXPECT_EQ(status = aclrtSetDevice(deviceId), 0);

    shmem_init_attr_t* attributes = new shmem_init_attr_t{rankId, n_ranks, testGlobalIpport, local_mem_size, {0, SHMEM_DATA_OP_MTE, 120, 120, 120}};
    status = shmem_init_attr(attributes);
...
void TestShmemInitInvalidRankId(int rankId, int n_ranks, uint64_t local_mem_size) {
    int erankId = -1;
    uint32_t deviceId = rankId % testGNpuNum + testFirstNpu;
    int status = SHMEM_SUCCESS;
    EXPECT_EQ(aclInit(nullptr), 0);
    EXPECT_EQ(status = aclrtSetDevice(deviceId), 0);
    shmem_init_attr_t* attributes;
    shmem_set_attr(erankId, n_ranks, local_mem_size, testGlobalIpport, &attributes);
    status = shmem_init_attr(attributes);
...
void TestShmemInitRankIdOverSize(int rankId, int n_ranks, uint64_t local_mem_size) {
    uint32_t deviceId = rankId % testGNpuNum + testFirstNpu;
    int status = SHMEM_SUCCESS;
    EXPECT_EQ(aclInit(nullptr), 0);
    EXPECT_EQ(status = aclrtSetDevice(deviceId), 0);
    shmem_init_attr_t* attributes;
    shmem_set_attr(rankId + n_ranks, n_ranks, local_mem_size, testGlobalIpport, &attributes);
    status = shmem_init_attr(attributes);
...
void TestShmemInitZeroMem(int rankId, int n_ranks, uint64_t local_mem_size) {
    //local_mem_size = 0
    uint32_t deviceId = rankId % testGNpuNum + testFirstNpu;
    int status = SHMEM_SUCCESS;
    EXPECT_EQ(aclInit(nullptr), 0);
...
void TestShmemInitInvalidMem(int rankId, int n_ranks, uint64_t local_mem_size) {
...
void TestShmemSetConfig(int rankId, int n_ranks, uint64_t local_mem_size) {
    uint32_t deviceId = rankId % testGNpuNum + testFirstNpu;
    int status = SHMEM_SUCCESS;
    EXPECT_EQ(aclInit(nullptr), 0);
    EXPECT_EQ(status = aclrtSetDevice(deviceId), 0);
