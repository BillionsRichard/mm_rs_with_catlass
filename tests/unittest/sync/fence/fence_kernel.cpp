...
extern "C" SHMEM_GLOBAL void fence_order2(uint64_t config, GM_ADDR addr, int rank_id, int rank_size) {
    ...
    if (rank_id == 1) {
        ...
        uint64_t seen_b = shmemi_load<uint64_t>(remote + sizeof(uint64_t));
...
void fence_order2_do(void* stream, uint64_t config, uint8_t *addr, int32_t rank_id, int32_t n_ranks) {
    fence_order2<<<1, nullptr, stream>>>(config, addr, rank_id, n_ranks);
}
