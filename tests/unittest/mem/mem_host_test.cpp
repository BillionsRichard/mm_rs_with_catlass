...
extern void TestPut(uint32_t block_dim, void* stream, uint8_t* gva, uint8_t* devPtr);
extern void TestGet(uint32_t block_dim, void* stream, uint8_t* gva, uint8_t* devPtr);

static void TestPutGet(aclrtStream stream, uint8_t *gva, uint32_t rank_id, uint32_t rank_size)
{
    int totalSize = 16 * (int)rank_size;
    ...
    TestPut(block_dim, stream, (uint8_t *)ptr, (uint8_t *)devPtr);
    ...
    TestGet(block_dim, stream, (uint8_t *)ptr, (uint8_t *)devPtr);
    ASSERT_EQ(aclrtSynchronizeStream(stream), 0);
    sleep(2);
...
void TestShmemMem(int rank_id, int n_ranks, uint64_t local_mem_size) {
    int32_t device_id = rank_id % test_gnpu_num + testFirstNpu;
    aclrtStream stream;
    TestInit(rank_id, n_ranks, local_mem_size, &stream);
    ASSERT_NE(stream, nullptr);

    TestPutGet(stream, (uint8_t *)shm::gState.heap_base, rank_id, n_ranks);
