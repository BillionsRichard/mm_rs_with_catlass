...
public:
    ...
    struct Params { 
        GemmCoord problemShape; uint32_t rankIdx; uint32_t rankSize; 
        GM_ADDR ptrA; LayoutA layoutA; GM_ADDR ptrB; LayoutB layoutB; GM_ADDR ptrSymmetric;
        ReduceScatterParams reduceScatterParams; BiasParams biasParams; DequantParams dequantParams;
        GM_ADDR ptrC_accum; LayoutC layoutC_accum; GM_ADDR ptrD_out; LayoutD layoutD_out; uint32_t commInterval;
        ...
        for (uint32_t commIdx = 0; commIdx < commLoops; ++commIdx) {
            ...
            if (aivIndex == 0 && aicoreIndex < commAicoreNum) {
                for (uint32_t commLoopIdx = aicoreIndex; commLoopIdx < commCoreLoops; commLoopIdx += commAicoreNum) {
                    ...
                    MatrixCoord blockOffset = commScheduler.template GetBlockOffset<ReduceScatter::RemoteCopyMode, ReduceScatter::RemoteCopyDirect>(commBlockCoord, layoutComm);
                    MatrixCoord actualCommBlockShape = commScheduler.template GetActualBlockShape<ReduceScatter::RemoteCopyMode, ReduceScatter::RemoteCopyDirect>(commBlockCoord, layoutComm);
                    ...
                    reduceScatter(blockShapeMN, offsetOut, offsetIn, actualCommBlockShape, gmC_accum, params.layoutC_accum, globalLoopIdx, remoteRankIdx % params.rankSize);
                }
            }
